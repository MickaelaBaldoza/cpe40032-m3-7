...
function PlayState:enter(params)
    ...
    self.keyTaken = false
    self.paddlerecoverPoints=10000
    ...
    for i, ball in pairs(self.balls) do
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
               ...
               -- Grow paddle
               if self.score > self.paddlerecoverPoints then
                    
                if self.paddle.size > 0 and self.paddle.size <4 then
                    self.paddle.size= self.paddle.size + 1
                end
            

                -- multiply paddle recover points by 3
                self.recoverPoints = math.min(200000, self.paddlerecoverPoints * 3)
...
function PlayState:render()
    ...
    if self.keyTaken then
        love.graphics.draw(gTextures['main'], gFrames['powerups'][2], VIRTUAL_WIDTH - 60, 10)
    end
